...
class Fundamentals(NamedTuple):
    ...
    signal: Signal | None  # BS column → BUY/SELL/HOLD/None
    pe_trailing: float | None  # PET — trailing 12-month P/E
...
class SignalsReader:
    ...
    def __init__(self, path: Path) -> None:
        self.path = path
        self._cache_signals: dict[str, Signal | None] = {}
    ...
    def _resolve_path(self) -> Path | None:
        ...
        if cached is None and not self._missing_logged:
            log.warning(
                "signals CSV unavailable (local %s missing, GitHub fetch failed)", self.path
            )
    ...
    def read(self) -> dict[str, Signal | None]:
